...
// Package
package eup.mobi.example.customview.furiganaview;
...
public class FuriganaView extends View {
    ...
    private void init(@Nullable AttributeSet set) {
        ...
        if (set == null) {
            tp.setTextSize(getResources().getDimensionPixelSize(R.dimen.sp16));
            tp.setAntiAlias(true);
            tp.setColor(Color.parseColor("#C41313"));
            tp.setDither(true);
            tp.setStyle(Paint.Style.FILL);
            tp.setStrokeJoin(Paint.Join.ROUND);
        } else {
            TypedArray ta = getContext().obtainStyledAttributes(set, R.styleable.fv);

            int textColor = ta.getColor(R.styleable.fv_tp_color, Color.parseColor("#C41313"));
            int textSize = ta.getDimensionPixelSize(R.styleable.fv_tp_size, getResources().getDimensionPixelSize(R.dimen.sp16));
            boolean antiAlias = ta.getBoolean(R.styleable.fv_tp_anti_alias, true);
            boolean dither = ta.getBoolean(R.styleable.fv_tp_dither, true);
            tp.setTextSize(textSize);
            tp.setAntiAlias(antiAlias);
...
//    }
    ...
    protected void onMeasure(int width_ms, int height_ms) {
        ...
        // Set result
        setMeasuredDimension(wnew, hnew < getSuggestedMinimumHeight() ? getSuggestedMinimumHeight() : hnew);
    }
